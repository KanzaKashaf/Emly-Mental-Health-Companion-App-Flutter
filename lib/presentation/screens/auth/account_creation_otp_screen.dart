...
class AccountCreationOtpScreen extends StatefulWidget {
  const AccountCreationOtpScreen({super.key});

  @override
  State<AccountCreationOtpScreen> createState() =>
      _AccountCreationOtpScreenState();
}

class _AccountCreationOtpScreenState extends State<AccountCreationOtpScreen> {
  ...
  void didChangeDependencies() {
    ...
    // Signup already auto-sends OTP, so start cooldown immediately.
  ...
  Future<void> _verifyOtp() async {
    ...
    if (email == null || email.trim().isEmpty) {
      _showError('Email is missing. Please sign up again.');
      return;
    }
  ...
  Future<void> _routeAfterEmailVerified() async {
    try {
      ...
      // Your current UserModel may not have needsProfileCompletion yet.
      // So for now, route to Home after successful email verification.
      ...
      // If profile refresh fails after verification, still send user to Home.
      // Home already loads profile and has retry behavior.
      Navigator.pushNamedAndRemoveUntil(
        context,
        AppRoutes.home,
        (route) => false,
    ...
    return KeyboardSafeScaffold(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'We have sent code verification to $emailText.\n'
            'Enter code below.',
            style: TextStyle(
              fontFamily: 'Poppins',
              fontSize: 15.5,
